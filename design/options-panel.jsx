// options-panel.jsx — Standalone Options panel (renamed from Tweaks).
// Bottom-LEFT floating panel. Works without any host iframe — opens with the
// "Options" button in the corner, closes with ✕. State lives in localStorage.

const __OPTIONS_STYLE = `
...
// ── OptionsPanel — standalone ───────────────────────────────────────────────
function OptionsPanel({ title = 'Options', children }) {
  const [open, setOpen] = React.useState(false);
  const dragRef = React.useRef(null);
  const offsetRef = React.useRef({ x: 20, y: 20 });
  const PAD = 16;
  ...
  return (
    <>
      ...
      {open && (
        <div ref={dragRef} className="opt-panel" data-noncommentable=""
          ...
          <div className="opt-hd" onMouseDown={onDragStart}>
            ...
            <button className="opt-x" aria-label="Close options"
...
// ── Layout helpers ──────────────────────────────────────────────────────────
function OptSection({ label, children }) {
  return (<><div className="opt-sect">{label}</div>{children}</>);
}

function OptRow({ label, value, children, inline = false }) {
  return (
...
// ── Controls ────────────────────────────────────────────────────────────────
function OptSlider({ label, value, min = 0, max = 100, step = 1, unit = '', onChange }) {
  return (
    <OptRow label={label} value={`${value}${unit}`}>
      <input type="range" className="opt-slider" min={min} max={max} step={step}
             value={value} onChange={(e) => onChange(Number(e.target.value))} />
    </OptRow>
  );
}

function OptToggle({ label, value, onChange }) {
  return (
    <div className="opt-row opt-row-h">
      <div className="opt-lbl"><span>{label}</span></div>
      <button type="button" className="opt-toggle" data-on={value ? '1' : '0'}
...
function OptRadio({ label, value, options, onChange }) {
...
function OptSelect({ label, value, options, onChange }) {
  return (
    <OptRow label={label}>
      <select className="opt-field" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => {
...
Object.assign(window, {
  OptionsPanel, OptSection, OptRow,
  OptSlider, OptToggle, OptRadio, OptSelect,
  // Backwards-compat aliases — clara-tweaks references TweaksPanel etc.
  TweaksPanel: OptionsPanel,
  TweakSection: OptSection,
  TweakRow: OptRow,
  TweakSlider: OptSlider,
  TweakToggle: OptToggle,
  TweakRadio: OptRadio,
  TweakSelect: OptSelect,
});
